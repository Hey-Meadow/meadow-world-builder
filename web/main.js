...
// Meadow iridescent uniforms (set per frame).
uniform float u_time;          // seconds since start
uniform float u_iri_strength;  // 0 = original colour, 1 = full iridescent
uniform vec3  u_scene_center;  // PLY centroid in world space
...
void main () {
    ...
    // ---- Meadow iridescent colour stack ----------------------------------
    vec4 origColor = vec4(
        float((cov.w) & 0xffu),
        float((cov.w >> 8) & 0xffu),
    ...
    float edge_band = exp(-pow(ndotv - 0.55, 2.0) / 0.025);
    float rim_band  = exp(-pow(ndotv - 0.05, 2.0) / 0.01);
    float grazing_w = clamp(edge_band + rim_band, 0.0, 1.0);
    vec3 chrome = (1.0 - grazing_w) * metallic_base + grazing_w * rainbow;
    chrome += 0.5 * pow(ndotv, 16.0);
    chrome = clamp(chrome, 0.0, 1.0);

    vec3 candy = clamp(rainbow + 0.05, 0.0, 1.0);
    vec3 iri = mix(candy, chrome, u_metallic_mix);
    vec3 finalRgb = mix(origColor.rgb, iri, u_iri_strength);

    vColor = clamp(pos2d.z/pos2d.w+1.0, 0.0, 1.0) * vec4(finalRgb, origColor.a);
    vPosition = position;
...
async function main() {
    ...
    // Meadow default: load Oatchi from our repo's assets/demos/ via relative path.
    // Override with ?url=https://.../something.ply for any other file.
    const url = new URL(
        params.get("url") || "../assets/demos/oatchi.ply",
        location.href,
    );
    ...
    const u_scene_center  = gl.getUniformLocation(program, "u_scene_center");
    const u_camera_pos    = gl.getUniformLocation(program, "u_camera_pos");
    ...
    const frame = (now) => {
        ...
        if (vertexCount > 0) {
            ...
            const tSec = (performance.now() - window.iriState.t0) / 1000.0;
            gl.uniform1f(u_time, tSec);
            gl.uniform1f(u_iri_strength, window.iriState.strength);
